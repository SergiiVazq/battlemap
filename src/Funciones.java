...
public class Funciones {
    ...
    public static int enemigodispara(String[][] map, int vidajugador) {
        Random mov = new Random();
            for (int i = 0; i < map.length; i++) {
                for (int j = 0; j < map[0].length; j++) {
                    if (map[i][j].equals("E")) {
                        if (map[i - 1][j].equals("J")) {
                            int prob = mov.nextInt(0, 100);
                            if (prob < 80) {
                                vidajugador--;
                                System.out.println("\n El enemigo te dio un balazo pierdes una vida");
                            }
                        } else if (map[i][j + 1].equals("J")) {
                            int prob = mov.nextInt(0, 100);
                            if (prob < 80) {
